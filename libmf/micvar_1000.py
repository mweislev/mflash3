...
class plotfile(flashfile):
    def __init__(self, filename, filter_nonleafs=True, memorize=True):
        flashfile.__init__(self, filename)
        nodetype = np.array(flashfile.read(self, 'node type'))
        self.__nodeselect = (nodetype==1) if filter_nonleafs else (nodetype==nodetype)
    ...
    def __contains__(self, key_in):
        ...
        return result
    def blockshape(self):
        nblocks = np.sum(self.__nodeselect)
        ...
        return blshape
    def blockid(self):
        dummy = self.__nodeselect[self.__nodeselect]
        blkid = np.arange(len(dummy))
        return blkid
    def nodeid(self):
        nodeid = np.arange(len(self.__nodeselect))
        return nodeid[self.__nodeselect]
    def get(self, key_in, default=None, default_shape=None):
        try:
            dataset = self.__getitem__(key_in)
        except KeyError:
            if default is None:
                raise
            ...
            return dataset
    def learnvar(self, key, upn_form, force=True):
        if key in self.list():
            if force:
                print('WARNING: Overriding plotfile variable "%s" with formula!'%key)
                self.__formulae[key] = upn(upn_form, str_eval=self.__getitem__)
            else:
                print('WARNING: Ignoring formula "%s" (conflicting plotfile variable)!'%key)
        else:
            self.__formulae[key] = upn(upn_form, str_eval=self.__getitem__)
